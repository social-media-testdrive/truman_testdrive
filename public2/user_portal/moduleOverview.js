...
// classReflectionResponses - json, data.classReflectionResponses from /classReflectionResponses/:classId/:modId
function countResponsesToPrompt(prompt, classReflectionResponses) {
    let responseCount = 0;
    for (const username in classReflectionResponses) {
        for (const attempt of classReflectionResponses[username]) {
            const responseToPrompt = attempt.answers.find(question => question.prompt === prompt && question.type == 'written' && question.writtenResponse !== "");
            if (responseToPrompt) {
                responseCount++;
                break;
            }
...
function getCheckboxLabelArray(reflectionJsonData, modName, question) {
    return Object.values(reflectionJsonData[modName][question].checkboxLabels || reflectionJsonData[modName][question].radioLabels);
}

// Parse the class's checkbox-type responses to a question into a format that can be readily inserted into a bar chart.
// Inputs:
// prompt - string, exact copy of the prompt
...
function parseClassCheckboxSelectionsForQuestion(prompt, numberOfCheckboxes, classReflectionResponses) {
    const checkboxSelections = [];
    for (let i = 0; i < numberOfCheckboxes; i++) {
        checkboxSelections.push(0);
    }
    for (const username in classReflectionResponses) {
        const attempts = classReflectionResponses[username];
        const latestAttempt = attempts[attempts.length - 1];
        const responseToPrompt = latestAttempt.answers.find(question => question.prompt === prompt && question.type == 'checkbox');
        // checkboxes responses are recorded in the binary format of the number.
        // Ex: 110 =
        // [√] checkbox label 1
        // [√] checkbox label 2
        // [ ] checkbox label 3
        let checkboxBinary = responseToPrompt.checkboxResponse;
...
function parseClassRadioSelectionsForQuestion(prompt, numberOfRadios, classReflectionResponses, chartLabelArray) {
    const checkboxSelections = [];
    ...
    for (const username in classReflectionResponses) {
        const attempts = classReflectionResponses[username];
        const latestAttempt = attempts[attempts.length - 1];
        const responseToPrompt = latestAttempt.answers.find(question => question.prompt === prompt && question.type == 'radio');
        const index = chartLabelArray.indexOf(responseToPrompt.radioSelection.trim());
        checkboxSelections[index]++;
    }
    return checkboxSelections;
}

function appendGroupedCheckboxTypeHtml(itemNumber, questionData) {
        ...
        <div class="ui items">
            ...
            <div class="ui fluid card">
                <div class="img post image">
                    <img src="${cdn}${questionData.groupPosts[itemNumber].postImage}" style="max-width:100%">
                </div>
                <div class="content">
                    <div class="description">${questionData.groupPosts[itemNumber].postDescription}</div>
                </div>
            </div>
            <div class="ui image" style="margin-left: 20px;">
...
function appendCheckboxTypeHtml(questionNumber, questionData, element) {
    $(element).append(`
        <div class="row">
...
function appendSectionLabels(modReflectionData) {
    ...
    if (written) {
        $("#openEndedResponses").append(`<h3 style="text-decoration: underline;">Students' answers to the open-ended questions:</h3>`);
    }
    if (checkbox) {
        $("#checkboxResponses").append(`<h3 style="text-decoration: underline;">Students' answers to the checkbox questions:</h3>`);
    }
    if (checkboxGrouped) {
        $("#groupedCheckboxResponses").append(`<h3 style="text-decoration: underline;">Students' answers to the grouped checkbox questions:</h3>`);
    }
    if (radio) {
        $("#radioResponses").append(`<h3 style="text-decoration: underline;">Students' answers to the radio questions:</h3>`);
    }
};
...
async function visualizeStudentReflectionData(modName, classId, classSize) {
  ...
  `);
    ...
    const dbData = await $.get(`/classReflectionResponses/${classId}/${modName}`);
    ...
    for (const questionNumber in modReflectionData) {
        ...
        switch (questionData.type) {
            ...
            case "checkbox":
                {
                    ...
                    const checkboxData = parseClassCheckboxSelectionsForQuestion(questionData.prompt, numberOfCheckboxes, classReflectionResponses);
                    appendCheckboxTypeHtml(questionNumber, questionData, "#checkboxResponses");
            ...
            case "checkboxGrouped":
                {
                    ...
                    const allGroupsCheckboxData = parseClassCheckboxSelectionsForQuestion(questionData.prompt, totalNumberOfCheckboxes, classReflectionResponses);
                    for (let i = 0; i < numberOfGroups; i++) {
                        ...
                        appendGroupedCheckboxTypeHtml(i, questionData);
            ...
            case "radio":
                {
                    const chartLabelArray = getCheckboxLabelArray(reflectionJsonData, modName, questionNumber);
                    const numberofRadios = chartLabelArray.length;
                    const radioData = parseClassRadioSelectionsForQuestion(questionData.prompt, numberofRadios, classReflectionResponses, chartLabelArray);
                    appendCheckboxTypeHtml(questionNumber, questionData, "#radioResponses");
                    const chartId = `#chart${questionNumber}`;
                    createCheckboxTypeChart(chartId, chartLabelArray, classSize, radioData);
                    break;
                }
        }
    }
...
async function visualizeFreeplayActivity(modName, classId, classSize) {
    ...
    $('#exploreHeading').append(`<h2>Exploring the Timeline</h2>`);
