...
exports.getGuest = async(req, res, next) => {
    ...
    if (!process.env.PRO_MONGODB_URI && !process.env.PRO_MONGOLAB_URI) {        // If no MongoDB, just redirect to intro without creating a user
    ...
    try {
        ...
        // If there's any MongoDB error, just redirect to intro
        console.log('MongoDB error in getGuest, redirecting to intro:', err.message);
        return res.redirect('/intro/' + req.params.modId);
    }
};
/*
...
// Add these functions after the existing ones (around line 380):

/**
 * POST /advancedlitInterest
 * Update user with the advancedlit topic selection.
 */
exports.postAdvancedlitInterestSelection = async(req, res, next) => {
    try {
        const user = await User.findById(req.user.id).exec();
...
exports.getAdvancedlitTopic = async(req, res) => {
    try {
        const user = await User.findById(req.user.id).exec();
        res.set('Content-Type', 'application/json; charset=UTF-8');
        res.send({ advancedlitTopic: user.advancedlitTopic });
    } catch (err) {
        next(err);
    }
};
