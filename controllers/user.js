...
exports.postAdvancedlitInterestSelection = (req, res) => {
  const { selectedInterest } = req.body;

  if (!selectedInterest) {
    return res.status(400).json({ error: 'No interest selected' });
  }

  // TODO: Save selectedInterest to the database or session here
  console.log(`User selected Advanced Lit interest: ${selectedInterest}`);

  res.status(200).json({ message: 'Interest recorded successfully' });
};

exports.getAdvancedlitTopic = (req, res) => {
  // Example: render a page or send data related to the "Advanced Literacy" topic
  res.status(200).json({ topic: 'Advanced Literacy', description: 'This is the advanced literacy topic page.' });
};
